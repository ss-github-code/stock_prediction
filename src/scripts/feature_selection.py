...
def get_shap_importances(model, X_val, features):
...
class FeatureSelector:
    def __init__(self, data_handler, max_num_features):
        self.data_handler = data_handler
        self.select_features_using_xgboost(max_num_features)

    def select_features_using_xgboost(self, max_num_features):
        ...
        feature_importance_df = get_shap_importances(xgb_best, X_val, data_handler.features)
        feature_importance_dict = feature_importance_df.abs().sum().sort_values(ascending=False).to_dict()
